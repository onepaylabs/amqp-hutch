...
AMQPHutch.prototype.publishToExchange = function (exchange, type, options, payload, callback) {
  ...
  if (channel){
    channel.publish(exchange, '', new Buffer(JSON.stringify(payload)), options.publish);
...
AMQPHutch.prototype._getChannelByQueue = function(queue){
  for(var id in this.channels) {
    ...
    if(id === PUBLISH_CHANNEL) return;
    if(this.channels[id].queue === queue) return this.channels[id];
  }
};
