  ...
  this._conn.createChannel(function(err, channel) {
    ...
    channel.assertExchange(options.exchange.name, options.exchange.type, options.exchange, function(err, ok) {
      ...
      channel.assertQueue(options.queue.name, options.queue, function(err, ok) {
        ...
        channel.bindQueue(options.queue.name, options.exchange.name, options.routingKey || '#');
        channel.prefetch(options.queue.prefetch);

        // Store the channel instance
        channels[options.queue.name] = channel;

        channel.on('close', function() {
          self.emit('channelClosed', options.queue.name);
          delete channels[options.queue.name];
        });

        channel.consume(options.queue.name, function(message) {
